...
# local imports
from analyze_ontology import AnalyzeOntology
...
def calculate_precision(file):  # file has predicted parent child pairs.
    population_pairs = load_pkl(file)
    iterations = list(population_pairs.keys())  # list of indices(0 to Length-1) in populated_paris DF.
    analyze_ontology = AnalyzeOntology()

    pairs = []
    for iteration in iterations:    # iterate in DF and store populated parent child pairs.
        pairs.extend(population_pairs[iteration])
    pd_pairs = pd.DataFrame(pairs, columns=['Parent', 'Child'])
    pd_pairs = pd_pairs[~pd_pairs['Parent'].isin(classes_without_embedding)]    # remove classes without embeddings.
    pd_pairs = pd_pairs[~pd_pairs['Child'].isin(entities_without_embedding)]

    tp, fp, _, _, _ = analyze_ontology.get_stats(pd_pairs)
    precision = tp / (tp + fp)
...
def do_analysis(file):
    ...
    tp, fp, tp_list, fp_list, distances = analyze_ontology.get_stats(
        pd_pairs,
        allow_distance=0,
        match_only=None)
...
def calculate_distance(file):
    ...
    _, _, _, _, distance_distribution = analyze_ontology.get_stats(pd_pairs)
...
def plot_precision_all_models():
    analyze_ontoloty = AnalyzeOntology()
...
def main():
    ...
    Main function.
    """
    set_logging('./logs/analysis.log')

    precision, filename = calculate_precision('./data/scores/pairs.pkl')
    print(f'Precision of file: {filename} is: {precision}')
    sys.exit()
